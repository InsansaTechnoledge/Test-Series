...
const ContestCard = ({ contest, setContest, theme }) => {
  ...
  const handlePause = async (contestItem) => {
    console.log("Pausing contest:", contestItem.id);
    setLoadingGoLive(prev => ({ ...prev, [contestItem.id]: true }));
  ...
  return (
    <div className="p-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {contestData?.map((contestItem) => {
          ...
          const isParticipationBased =
            contestItem.type === "participation_based";
          ...
          return (
            <div
              key={contestItem.id}
              className={`group relative backdrop-blur-sm shadow-lg hover:shadow-2xl transition-all duration-500 rounded-3xl overflow-hidden border transform hover:-translate-y-2 ${theme === 'dark'
            ...
            >
              ...
              <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${isParticipationBased
              ...
              {/* Header */}
              <div
                className={`relative px-6 py-4 ${theme === 'dark'
                  ? isParticipationBased
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span
                        className={`text-xs font-bold tracking-wider uppercase ${theme === 'dark'
                ...
                <div className="flex justify-center">
                  <div
                    className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold border ${status.color} shadow-sm`}
                  >
              ...
              {/* Content */}
              <div className={`relative p-6 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'
                }`}>
                <h3 className={`text-xl font-bold mb-3 line-clamp-2 leading-tight ${theme === 'dark' ? 'text-white' : 'text-gray-900'
                  }`}>
              <div
              ...
              >
                ...
                {contestItem.description && (
                  <p className={`text-sm mb-5 line-clamp-3 leading-relaxed ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'
                    }`}>
                  <p
                ...
                <div className="space-y-4">
                  <div className={`flex items-center gap-3 p-3 rounded-xl ${theme === 'dark' ? 'bg-gray-700/30' : 'bg-gray-100/50'
                    }`}>
                  ...
                  {isParticipationBased ? (
                    contestItem.validity.start && contestItem.validity.end && (
                      <div className={`p-4 rounded-xl border-2 ${theme === 'dark'
                        ...
                        <div className={`text-xs font-bold mb-2 flex items-center gap-2 ${theme === 'dark' ? 'text-indigo-300' : 'text-indigo-700'
                          }`}>
                          <Calendar className="w-3 h-3" />
                          VALID PERIOD
                        </div>
                        <div className={`text-sm font-semibold ${theme === 'dark' ? 'text-indigo-200' : 'text-indigo-800'
                          }`}>
                          {formatDate(contestItem.validity.start)} - {formatDate(contestItem.validity.end)}
                        </div>
                        {!isValidityActive(contestItem.validity) && (
                          <div className={`text-xs mt-2 flex items-center gap-1 ${theme === 'dark' ? 'text-amber-300' : 'text-amber-600'
                            }`}>
                    ...
                    contestItem.schedule && (
                      <div className={`p-4 rounded-xl border-2 ${theme === 'dark'
                        ...
                        <div className={`text-xs font-bold mb-2 flex items-center gap-2 ${theme === 'dark' ? 'text-purple-300' : 'text-purple-700'
                          }`}>
                          <Clock className="w-3 h-3" />
                          SCHEDULED FOR
                        </div>
                        <div className={`text-sm font-semibold ${theme === 'dark' ? 'text-purple-200' : 'text-purple-800'
                          }`}>
                  ...
                  {isParticipationBased
                    ? contestItem.validity.start &&
                      contestItem.validity.end && (
                        <div
                        ...
                        >
                          ...
                          >
                            {formatDate(contestItem.validity.start)} -{" "}
                            {formatDate(contestItem.validity.end)}
                          </div>
                          {!isValidityActive(contestItem.validity) && (
                            <div
                  ...
                  {(!contestItem.go_live || (contestItem.go_live && !canMakeGoLive)) && (
                    <div className="pt-2">
                      <button
                        ...
                        // disabled={loadingGoLive[contestItem.id] || (contestItem.go_live && !canMakeGoLive)}
    ...
    disabled:transform-none disabled:shadow-none`}
                        ...
                        disabled={
                          !canMakeGoLive || loadingGoLive[contestItem.id]
                        }
                        className={`w-full py-3 px-4 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 ${
                          canMakeGoLive
                            ? theme === "dark"
                      ...
                      >
                        ...
                        {loadingGoLive[contestItem.id] ? (
                          <>
                            ...
                            {contestItem.go_live ? 'Pausing...' : 'Going Live...'}
                          </>
                        ) : (
                          <>
                            <Play className="w-4 h-4" />
                            {contestItem.go_live && !canMakeGoLive
                              ? 'Pause The Contest'
                              : canMakeGoLive
                                ? 'Go Live'
                                : 'Cannot Go Live'}
                      ...
                      {!canMakeGoLive && (
                        ...
                        >
                          Contest must be within valid period or scheduled time
                          to go live
                        </p>
                      )}
                    </div>
      ...
      {contestData.length === 0 && (
        <div className={`text-center py-12 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'
          ...
          <div className={`w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center ${theme === 'dark' ? 'bg-gray-800' : 'bg-gray-100'
            }`}>
        <div
