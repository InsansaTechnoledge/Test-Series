...
const ExamListPage = () => {
  ...
  if (!canAccessPage) {
    return (
      <div className="flex items-center justify-center">
        <div className={`text-center px-4 py-3 my-auto ${theme === 'dark'
          ? 'bg-gray-800 text-gray-200'
          : 'bg-red-100 text-gray-600'
          }`}>
          <h1 className={`text-3xl font-bold mb-4 ${theme === 'dark' ? 'text-red-400' : 'text-red-600'
            }`}>Access Denied</h1>
  ...
  const handleAddQuestion = (examId) => {
    if (canCreateExam) { navigate(`/institute/create-exam/${examId}`); }
  };
  ...
  if (error) {
    return (
      <div className={`min-h-screen flex items-center justify-center ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'
        ...
        <div className="text-center space-y-4">
          <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto ${theme === 'dark' ? 'bg-gray-800' : 'bg-red-100'
            }`}>
            <BookOpen className={`w-8 h-8 ${theme === 'dark' ? 'text-red-400' : 'text-red-600'
              }`} />
          </div>
          ...
          <div className="space-y-10">
            {Object.entries(groupedExams).map(([batchName, batchExams]) => (
              <div key={batchName} className="space-y-5">
                {/* Batch Header */}
                <div className={`flex items-center space-x-3 pb-4 border-b px-2 py-2 rounded ${theme === 'dark'
                  ? 'bg-indigo-400'
                  : 'border-indigo-200 bg-indigo-600 text-white'
                  }`}>
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center shadow ${theme === 'dark' ? 'bg-gray-100' : 'bg-indigo-100'
                    }`}>
                    <Users className={`w-4 h-4 ${theme === 'dark' ? 'text-indigo-600' : 'text-indigo-600'
                      }`} />
                  </div>
                  <div>
                ...
                <div className="grid sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  {batchExams.map((exam) => (
                    <div
                      key={exam.id}
                      className={`w-full max-w-md mx-auto rounded-2xl shadow-lg transition-all duration-300 overflow-hidden transform hover:-translate-y-1 hover:scale-[1.01] border-2 ${theme === 'dark'
                        ? exam.go_live
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between mb-4 p-4">
                        <div
                          className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold shadow-sm ${exam.go_live
                            ? theme === 'dark'
                        ...
                        {canDeleteExams && (<button
                          className={`transition-colors duration-200 p-1 ${theme === 'dark' ? 'text-gray-500 hover:text-red-400' : 'text-gray-400 hover:text-red-500'
                            }`}
                          onClick={() => {
                      ...
                      {/* Exam Title */}
                      <div className="px-4">
                        <h3 className={`text-lg font-bold py-2 px-4 rounded-xl text-center mb-4 ${theme === 'dark' ? 'text-indigo-300' : 'text-indigo-600'
                          }`}>
                          {exam.name}
                        </h3>
                        ...
                        <div className="space-y-2 mb-4">
                          ...
                          <div className="flex items-center justify-between text-sm">
                            ...
                            <span className={`font-medium flex items-center space-x-1 ${exam.go_live
                      ...
                      {/* Action Buttons */}
                      <div className={`pt-3 border-t px-4 pb-5 ${theme === 'dark' ? 'border-gray-600' : 'border-gray-200'
                        }`}>
                        {!exam.go_live ? (
                          (pendingExams.some((p) => p.id === exam.id) && canCreateExam) ? (
                            ...
                            <button
                              ...
                              className={`w-full py-2.5 px-4 rounded-full font-semibold text-sm flex items-center justify-center gap-2 transition duration-200 ${updatingId === exam.id
                                ? theme === 'dark'
                          ...
                          <div className="space-y-2">
                            <div className={`w-full py-2 px-3 rounded-lg border font-medium text-sm flex items-center justify-center gap-2 ${theme === 'dark'
