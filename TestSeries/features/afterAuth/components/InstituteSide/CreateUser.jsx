...
const CreateUser = () => {
  ...
  const onsubmitForm = async () => {
    ...
    if (isLimitExceeded) {
      setError((prev) => ({
        ...prev,
        form: "User creation limit exceeded. Please upgrade your plan.",
      }));
      return;
    }
      ...
      <div className="max-w-6xl mx-auto px-6 -mt-8 relative z-20 pb-12">
        <div
          className={` rounded-3xl shadow-2xl  overflow-hidden  ${
            theme === "light" ? "  bg-gray-100" : "bg-gray-800 text-gray-100"
          } `}
        >
          {/* Profile Section */}
          <div
            className={` p-8  {theme === 'light' ? '  border-gray-100' : 'bg-gray-800'} `}
          >
            <div className="flex flex-col md:flex-row gap-8 items-center">
              <div className="relative group">
                <img
              ...
              <div className="flex flex-col items-center md:items-start">
                <label
                  className={`py-3 px-6 rounded-2xl cursor-pointer text-sm font-bold transition-all duration-300 ${
                    isLimitExceeded
                      ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                      : "bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:shadow-xl hover:scale-105 transform"
                  }`}
                ...
                <span
                  className={`text-sm text-gray-600 mt-2 text-center md:text-left ${
          ...
          {isLimitExceeded && (
            <div className="mx-8 mt-6">
              <p
                className={`text-center text-sm  px-4 py-2 rounded-xl shadow-sm backdrop-blur-sm  ${
                  theme === "light"
                    ? "bg-red-100 border text-red-600 border-red-200"
                    : "bg-red-600 text-gray-100"
                }`}
              >
                You've reached your user creation limit.{" "}
                ...
                <div className="flex gap-3">
                  <div className="relative flex-1">
                    ...
                    <button
                      ...
                      disabled={isLimitExceeded}
                      onClick={() =>
                        !isLimitExceeded && setShowPassword(!showPassword)
                      }
                    >
                      {showPassword ? <EyeOff size={20} /> : <Eye size={20} />}
                    </button>
                  ...
                  >
                    <RefreshCcw
                      className={`w-5 h-5 ${
                        isLimitExceeded ? "text-gray-400" : "text-indigo-600"
                      }`}
                    />
                  </button>
                </div>
            ...
            <div className="mt-12">
              <div
                className={` rounded-3xl p-8   ${
                  theme === "light"
                    ? "bg-gradient-to-r from-gray-50 to-indigo-50 border-gray-100  border-2 shadow-2xl"
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() =>
                    !isLimitExceeded && setBatchesVisible(!batchesVisible)
                  }
                  disabled={isLimitExceeded}
