    ...
    return (
        <div className="max-w-6xl mx-auto p-6 min-h-screen">
            ...
            {sortedData.length >= 2 && !loading && (
                <div className="mb-8 border-3 border-amber-600  py-8 rounded-4xl">
                    <h2 className="text-2xl font-bold text-center mb-6 text-gray-800">Top Performers </h2>
                                        ...
                                        >
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    {getRankIcon(rank)}
                                                    <span className={`font-bold text-lg ${rank <= 3 ? (rank === 1 ? 'text-white' : 'text-gray-800') : 'text-gray-700'}`}>
                                                        #{rank}
                                                    </span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className={`font-semibold text-lg ${rank === 1 ? 'text-white' : 'text-gray-800'}`}>
            ...
            {sortedData.length === 0 && exam && !loading && !error && (
                <div className="text-center py-12 bg-white rounded-2xl shadow-lg">
                    ...
                    <p className="text-gray-500">No student results available for this exam.</p>
                </div>
            )}
            ...
            {!exam && exams.length === 0 && (
                <div className="text-center py-12 bg-white rounded-2xl shadow-lg">
                    ...
                    <p className="text-gray-500">Please wait while we load the available exams...</p>
                </div>
            )}
        </div>
    )
