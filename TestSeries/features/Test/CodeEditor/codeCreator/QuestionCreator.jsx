...
import { Plus, Trash2, Save, Code, ChevronLeft, ChevronRight, BookOpen, Trophy, CheckCircle, XCircle } from 'lucide-react';
...
export default function QuestionCreator() {
  ...
  const submitContest = async () => {
    const payload = {
      contestId: contestId,
      questions: selectedQuestions.map(({id, marks, difficulty}) => ({ coding_question_id: id, marks, difficulty}))
    }
  ...
  const themeClasses = {
    ...
    primaryHover: theme === 'dark' ? 'hover:bg-indigo-300' : 'hover:bg-indigo-700',
    primaryText: theme === 'dark' ? 'text-indigo-400' : 'text-indigo-600',
    button: theme === 'dark' ? 'bg-gray-800 hover:bg-gray-700 text-gray-200' : 'bg-gray-100 hover:bg-gray-200 text-gray-700',
    input: theme === 'dark' ? 'bg-gray-800 border-gray-600 text-gray-200' : 'bg-white border-gray-300 text-gray-900',
    shadow: theme === 'dark' ? 'shadow-lg shadow-gray-900/20' : 'shadow-lg shadow-gray-200/20',
  };
  ...
  return (
    <div className={`min-h-screen ${themeClasses.bg} transition-colors duration-200`}>
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Header */}
        <div className={`${themeClasses.cardBg} ${themeClasses.border} border rounded-2xl p-6 mb-8 ${themeClasses.shadow}`}>
          <div className="flex items-center gap-3 mb-2">
            <BookOpen className={`w-8 h-8 ${themeClasses.primaryText}`} />
            <h1 className={`text-3xl font-bold ${themeClasses.text}`}>Contest Question Creator</h1>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          <div className="xl:col-span-2 space-y-6">
            {/* Filter Section */}
            <div className={`${themeClasses.cardBg} ${themeClasses.border} border rounded-xl p-6 ${themeClasses.shadow}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <label className={`${themeClasses.text} font-semibold text-lg`}>Difficulty Level:</label>
                  <select
                ...
                <div className={`px-4 py-2 rounded-lg ${themeClasses.button} font-medium`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {questions?.map((q) => (
                <div
                  key={q.id}
                  className={`${themeClasses.cardBg} ${themeClasses.border} border rounded-xl p-6 cursor-pointer transition-all duration-200 hover:scale-105 ${themeClasses.shadow} hover:shadow-xl ${
                    selectedId === q.id ? `ring-2 ring-indigo-500 ${theme === 'dark' ? 'bg-indigo-950' : 'bg-indigo-50'}` : ''
                  }`}
                  onClick={() => {
                    setSelectedId(q.id);
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <h3 className={`font-bold text-lg ${themeClasses.text} leading-tight`}>{q.title}</h3>
            ...
            {/* Pagination */}
            <div className={`${themeClasses.cardBg} ${themeClasses.border} border rounded-xl p-4 ${themeClasses.shadow}`}>
              <div className="flex items-center justify-between">
                <button
          ...
          <div className="space-y-6">
            <div className={`${themeClasses.cardBg} ${themeClasses.border} border rounded-xl p-6 ${themeClasses.shadow} sticky top-6`}>
              <div className="flex items-center gap-3 mb-4">
                <Trophy className={`w-6 h-6 ${themeClasses.primaryText}`} />
                <h2 className={`text-xl font-bold ${themeClasses.text}`}>Selected Questions</h2>
                <span className={`px-3 py-1 text-sm font-bold rounded-full ${themeClasses.primary} text-white`}>
              ...
              {selectedQuestions.length === 0 ? (
                <div className={`text-center py-8 ${themeClasses.textMuted}`}>
                  ...
                  <p className="text-sm mt-1">Click on questions to add them to your contest.</p>
                </div>
              ) : (
                <div className="space-y-4 max-h-96 overflow-y-auto">
                  {selectedQuestions.map((q, index) => (
                    <div key={q.id} className={`${themeClasses.border} border rounded-lg p-4 space-y-3`}>
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <h4 className={`font-bold ${themeClasses.text} text-sm`}>
                            {index + 1}. {q.title}
                          </h4>
                      ...
                      <div className="flex items-center gap-2">
                        <label className={`text-sm ${themeClasses.textSecondary} font-medium`}>Marks:</label>
                        <input
                          type="number"
                      ...
                      {q.test_cases?.length > 0 && (
                        <div className={`text-xs ${themeClasses.textMuted} space-y-1`}>
                          <p className="font-semibold">Test Case Distribution:</p>
                            ...
                            {q.test_cases.slice(0, 3).map((_, i) => {
                              ...
                              return (
                                <div key={i} className="flex justify-between">
                                  ...
                                  <span className="font-mono">{perCaseMark.toFixed(2)} marks</span>
                                </div>
                              );
                            })}
        ...
        {fullDetails && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className={`${themeClasses.cardBg} rounded-2xl p-8 max-w-4xl w-full max-h-[80vh] overflow-y-auto ${themeClasses.shadow}`}>
              <div className="flex items-start justify-between mb-6">
                <div>
                  <h2 className={`text-2xl font-bold ${themeClasses.text} mb-2`}>
                    {fullDetails.title}
                  </h2>
                  <span className={`px-3 py-1 text-sm font-medium rounded-full ${getDifficultyColor(fullDetails.difficulty)}`}>
              ...
              <div className="flex gap-4">
                <button
                  ...
                  disabled={selectedQuestions.some((q) => q.id === fullDetails.id)}
                >
                  <Plus className="w-5 h-5" />
                  {selectedQuestions.some((q) => q.id === fullDetails.id) ? 'Already Added' : 'Add to Contest'}
                </button>
