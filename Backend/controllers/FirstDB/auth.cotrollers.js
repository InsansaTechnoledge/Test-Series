...
import { updateOrganization } from "./organization.controllers.js";
import { updateStudent } from "./student.controllers.js";
...
export const profileEdit = async (req, res) => {
  console.log("Profile Edit Request:", req.body);
  const  updatedData = req.body;

  if (!updatedData) {
    return new APIError(400, ["UserId and updatedData are required!!"]).send(res);
  }

  try {
    let response = null;
    if (req.user.role === 'organization') {
      response = await Organization.findByIdAndUpdate(req.user._id, {
        $set: updatedData
      }, { new: true });

    } else if (req.user.role === 'student') {
      response = await Student.findByIdAndUpdate(req.user._id, {
        $set: updatedData
      }, { new: true });
    }
    else {
      response = await User.findByIdAndUpdate(req.user._id, {
        $set: updatedData
      }, { new: true });
    }

    return new APIResponse(200, response, "Profile updated successfully").send(res);

  } catch (err) {
    console.log(err);
    return new APIError(500, ["Something went wrong while updating profile", err.message || ""]).send(res);
  }
}